...
public sealed class TussCsvParser : IProcedureFileParser
{
    public ProcedureSource Source => ProcedureSource.Tuss;
    ...
    public ParseResult Parse(Stream csvStream)
    {
        ...
        while ((line = reader.ReadLine()) is not null)
        {
            ...
            {
                skippedCount++;
                errors.Add($"linha {lineNumber}: valor inválido '{parts[2].Trim()}'");
                continue;
            }

            DateOnly? effectiveTo = null;
            if (parts.Length >= 5 && !string.IsNullOrWhiteSpace(parts[4].Trim()))
            {
                if (DateOnly.TryParseExact(parts[4].Trim(), "dd/MM/yyyy",
                        System.Globalization.CultureInfo.InvariantCulture,
            ...
            rows.Add(new ParsedProcedureRow(code, description, value, effectiveTo));
        }
